...
static OCHttpRequest *instance = nil;
...
+ (void)removeAllCaches {
    ...
    [OCFileManager clearAllCacheFile];
}

+ (void)cancelAllOperations {
    
    [[OCHttpRequest sharedInstance].sessionManager.operationQueue cancelAllOperations];
}

+ (void)setHeader:(void (^)(AFHTTPRequestSerializer *))handler {
    AFHTTPRequestSerializer *requestSerializer = [OCHttpRequest sharedInstance].sessionManager.requestSerializer;
...
+ (void)GET:(NSString *)url
    ...
    [OCHttpRequest requestMethod:HttpRequestTypeGET
...
+ (void)POST:(NSString *)url
    ...
    [OCHttpRequest requestMethod:HttpRequestTypePOST
                     cachePolicy:HttpRequestDefault
                             url:url
                          params:params
                         success:successHandler
...
constructingBodyWithBlock:(void (^)(id <AFMultipartFormData> formData))block
    ...
    [[OCHttpRequest sharedInstance].sessionManager POST:url parameters:params constructingBodyWithBlock:block success:^(NSURLSessionDataTask *task, id responseObject) {
        if (successHandler) {
            if (cachePolicy != HttpRequestDefault && responseObject) { // 缓存数据
                [OCFileManager wirteToFileWithFileName:[url MD5] content:responseObject];
            }
...
+ (void)requestMethod:(HttpRequestType)requestType
    ...
    if ([OCHttpRequest sharedInstance].timeoutInterval > 0) {
        [OCHttpRequest sharedInstance].sessionManager.requestSerializer.timeoutInterval = [OCHttpRequest sharedInstance].timeoutInterval;
    }
    ...
    if (cachePolicy == HttpRequestReturnCacheDataThenLoad ||
        ...
        NSString *dataStr = [OCFileManager readFileWithFileName:[url MD5]];
    ...
    switch (requestType) {
        case HttpRequestTypeGET: {   // GET请求
            [[OCHttpRequest sharedInstance].sessionManager GET:url parameters:params success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        ...
        case HttpRequestTypePOST: {  // POST请求
            ...
            [[OCHttpRequest sharedInstance].sessionManager POST:url parameters:params success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
...
+ (void)UploadImageWithUrl:(NSString *)URLString
    ...
    [[OCHttpRequest sharedInstance].sessionManager POST:URLString parameters: parameters constructingBodyWithBlock:^(id<AFMultipartFormData> formData) {
